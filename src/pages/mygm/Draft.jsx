...
export const Draft = () => {
  ...
  const runEmergencyDraft = async (stateData) => {
    ...
    Swal.fire({
      ...
      text: 'Tus directivos están cerrando automáticamente las negociaciones del Draft en tu ausencia...',
      icon: 'warning',
  ...
  const handlePlayerPick = async (wrestler) => {
    ...
    if (session.budget < cost) {
      return Swal.fire({
        title: 'Fondos Insuficientes', text: '¡No tienes presupuesto para fichar a esta superestrella!',
  ...
  const applyRosterPenaltyAndFinish = async () => {
    ...
    try {
      ...
      Swal.fire({ title: 'Ajuste Realizado', text: 'Tu plantilla ha sido ajustada automáticamente para cumplir con el mínimo de 10 superestrellas.', icon: 'info', confirmButtonColor: currentBrandColor });
      advanceTurn(newFinished);
    } catch (error) {
      Swal.fire('Error', 'No se pudieron aplicar los ajustes en la base de datos.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePlayerFinish = () => {
    if (myRoster.length < 10) {
      Swal.fire({
        ...
        text: 'ATENCIÓN: Tienes menos de 10 luchadores. Si te plantas ahora, el sistema venderá automáticamente a tu superestrella más cara y fichará a las más baratas hasta que alcances el mínimo de 10. ¿Aceptar?',
        icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonColor: '#3085d6',
        confirmButtonText: 'Sí, aplicar ajuste', cancelButtonText: 'Cancelar'
      }).then((result) => {
      ...
      Swal.fire({
        title: '¿Finalizar tu Draft?',
        text: '¿Seguro que quieres finalizar tu draft? Tu plantilla se cerrará y no podrás fichar más superestrellas.',
        icon: 'question', showCancelButton: true, confirmButtonColor: currentBrandColor, cancelButtonColor: '#d33',
        confirmButtonText: 'Sí, plantarme', cancelButtonText: 'Seguir fichando'
      }).then((result) => {
        if (result.isConfirmed) {
          const newFinished = [...finishedBrands, session.brand];
          setFinishedBrands(newFinished);
  ...
  const saveAndExitDraft = async () => {
    ...
    try {
      ...
      Swal.fire('Error', 'Hubo un error guardando los picks de la CPU', 'error');
      setLoading(false);
    }
  };
